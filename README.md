...
Repository-wide conventions for comments, TSDoc/JSDoc, Astro component `Props` interfaces, and TypeScript formatting are documented in [`coding-standards.instructions.md`](.github/instructions/coding-standards.instructions.md). In short, comments explain intent rather than restating code, exported data-layer functions document their parameters and return values, and ESLint enforces explicit types at exported TypeScript module boundaries.
...
The homepage supports filtering the catalog by one or more categories and by publisher. Filters are reflected in the URL so a filtered view can be bookmarked or shared. Category selections use repeated `category` parameters and the publisher uses one `publisher` parameter, for example:
...
The shared **Issue Triage** canvas (`.github/extensions/kanban-triage/`) loads open GitHub issues, highlights the three most recently active issues, and lets contributors add an issue to the current Copilot session context. It requires network access to the repository's GitHub Issues API.
